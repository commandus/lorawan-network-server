...
public:
    ...
    uint32_t freq100;               // freq number TX central frequency in MHz (float, Hz precision)
    uint32_t rfch;                  // Concentrator "RF chain" used for TX
    ...
    uint32_t frequencydeviation;    // FSK frequency deviation, Hz
    bool invertpolarization;    // Lora modulation polarization inversion
    uint32_t preamblesize;          // RF preamble size
